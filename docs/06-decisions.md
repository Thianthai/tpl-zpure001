...
## D17 — โลโก้และที่อยู่บริษัทพิมพ์เฉพาะใบที่อนุมัติแล้ว (2026-09-24)

ผู้ใช้แจ้งว่าเอกสารที่ยังไม่อนุมัติต้องไม่มีหัวจดหมายของบริษัท จึงซ่อน `Image1` (โลโก้) และ subform `CompAddress`

- ทั้งสองเป็น `draw` และ `subform` ซึ่ง XFA ผูก script กับตัวเองไม่ได้
  จึงสั่ง `presence` จาก field `PurchaseOrderNumber` ที่อยู่บน master page เดียวกัน (event `layout:ready`)
- `ZPURF002` ดูจาก `ApprovalNoteText` ว่าง เท่ากับยังไม่อนุมัติ (ABAP ส่งมาเฉพาะสถานะ A กับ B)
- `ZPURF001` ดูจาก `YY1_Approval_date_PDH` เกณฑ์เดียวกับข้อความ "อนุมัติผ่านระบบอิเล็กทรอนิกส์" ที่ฟอร์มใช้อยู่เดิม
- **ไม่ต้องแก้ ABAP**

## D18 — จำนวนเงินตัวอักษรของสกุลอื่น (2026-09-24)

`ZCL_PURE001_UTIL->amount_in_words` สำหรับสกุลที่ไม่ใช่ THB เปลี่ยนเป็นตัวพิมพ์ใหญ่ในวงเล็บ

| กรณี | ผล |
|---|---|
| ทศนิยมเป็นศูนย์ | `( SIXTY-EIGHT USD ONLY )` |
| มีเศษ | `( SIXTY-FOUR AND 56/100 USD )` ไม่มี ONLY |
| THB | `( สิบล้าน…บาทถ้วน )` ลงท้าย `บาทถ้วน` เมื่อเศษเป็นศูนย์ |

**แก้เพิ่ม 2026-09-25** — ผู้ใช้สั่งให้ครอบวงเล็บทุกสกุลเงินเหมือนฟอร์มมาตรฐาน
จึงย้ายการครอบวงเล็บออกมาไว้ท้าย method ที่เดียว ไม่ต้องเขียนซ้ำในแต่ละสาขา
...
## D21 — ไม่ทำตารางรูปภาพ โลโก้ฝังในฟอร์ม และไม่มีรูปลายเซ็น (2026-09-25)

ตรวจ `.xdp` ของฟอร์มมาตรฐานแล้วพบว่า **ไม่มีรูปไหนมาจากข้อมูลเลย**

| สิ่งที่คิดว่าต้องมีตารางรูป | ความจริงในฟอร์มมาตรฐาน |
|---|---|
| โลโก้บริษัท | `draw name="Image1"` มีรูป base64 **ฝังในไฟล์ `.xdp`** (Embed Image Data) ไม่มี binding · `ZPURF002` ได้รูปเดียวกันติดมาตอน clone |
| ลายเซ็นผู้อนุมัติ | ไม่มี image field เลย มีแต่ field `Approve_Name_sign` ที่ใช้ฟอนต์ลายมือ `SOV_Nophanit` พิมพ์ชื่อผู้อนุมัติ และตั้ง `presence="hidden"` ไว้ คือไม่พิมพ์ |

**ตัดสินใจ** — ยกเลิก `ZPURE001_GRPH` และ `ZE_PURE001_GRAPHIC_NAME` ไม่ต้องสร้าง
· field `CompanyLogo` / `ApprovedBySignature` (พร้อม MimeType และ FileName) ใน `ZR_PURE001_FDP` คงไว้ได้เพราะไม่มีใคร bind
· ถ้าวันหนึ่งอยากได้ลายเซ็น ทำได้สองทางโดยไม่ต้องมีตารางรูป
  คือเปิด `Approve_Name_sign` ให้ visible (bind `ApprovedByName` ไว้แล้ว) หรือฝังรูปในฟอร์มแบบเดียวกับโลโก้
